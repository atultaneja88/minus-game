...
// On connect — try to rejoin using stored session
socket.on('connect', () => {
...
function renderHint() {
  ...
  const msgs = {
    waiting:   '① Click a card to select it, then click "Discard Selected" — or call ⚡ SHOW.',
    discarded: '② Draw from deck, or click the glowing discard card to take it.',
...
function showRoundOv() {
  ...
  const isEnd=S.round>=S.max_rounds, isHost=S.my_sid===S.host_sid;
  ...
  S.players.forEach(p => {
    ...
    const hand=p.hand||[], hs=hand.map(c=>`${c.value}${SYM[c.suit]}`).join(' ');
  ...
  html+=`</tbody></table>`;
  if (isEnd) html+=`<button class="btn-cta" onclick="showGameEnd()">See Final Results →</button>`;
  else if (isHost) html+=`<button class="btn-cta" onclick="nextRound()">Next Round (${S.round+1}/${S.max_rounds}) →</button>`;
  else html+=`<p style="text-align:center;color:rgba(255,255,255,.5);font-size:.85rem">⏳ Waiting for host to start next round…</p>`;
...
function startConfetti() {
  ...
  draw();
  setTimeout(()=>{ if(confRAF){cancelAnimationFrame(confRAF);confRAF=null;} ctx.clearRect(0,0,W,H); },8000);
}
